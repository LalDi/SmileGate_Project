	...
	if (m_NowStatus == Status::Playing)
	{
		...
		if (timeGetTime() - m_MobSpawn > m_MobDelay && !b_OnBoss)
		{
			...
			case Stage::Stage1:	// 교황속성 주류 스테이지
				if (Temp >= 70)
					AddEnemy(Pos, Angle, EnemyState::The_Hierophant);	// 교황속성 확률 70%
				else if (Temp >= 85)
					AddEnemy(Pos, Angle, EnemyState::The_Hermit);		// 은둔자 속성 확률 15%
				else
					AddEnemy(Pos, Angle, EnemyState::The_Magician);		// 마법사 속성 확률 15%
				break;
			case Stage::Stage2:	// 은둔자속성 주류 스테이지
				if (Temp >= 70)
					AddEnemy(Pos, Angle, EnemyState::The_Hermit);		// 은둔자 속성 확률 70%
				else if (Temp >= 85)
					AddEnemy(Pos, Angle, EnemyState::The_Hierophant);	// 교황속성 확률 15%
				else
					AddEnemy(Pos, Angle, EnemyState::The_Magician);		// 마법사 속성 확률 15%
				break;
